...
import { getProductoById, updateProducto, getAlmacenes } from './AlmacenesService';
...
export const getTransferencias = async (filtros = {}) => {
  try {
    ...
    // Obtener los almacenes para asegurarnos de tener los nombres correctos
    const almacenesData = await getAlmacenes();
    
    // Enriquecer los datos de transferencias con nombres de almacenes si es necesario
    const transferenciasEnriquecidas = transferencias.map(transferencia => {
      // Si ya tiene los nombres, usarlos
      if (transferencia.almacenOrigenNombre && transferencia.almacenDestinoNombre) {
        return transferencia;
      }
      
      // Si no tiene los nombres, buscarlos
      const almacenOrigen = almacenesData.find(a => a.id === transferencia.almacenOrigenId);
      const almacenDestino = almacenesData.find(a => a.id === transferencia.almacenDestinoId);
      
      return {
        ...transferencia,
        almacenOrigenNombre: almacenOrigen ? almacenOrigen.nombre : 'Almacén no encontrado',
        almacenDestinoNombre: almacenDestino ? almacenDestino.nombre : 'Almacén no encontrado'
      };
    });
    
    return transferenciasEnriquecidas;
  } catch (error) {
